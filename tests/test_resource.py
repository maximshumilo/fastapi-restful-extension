...
from fastapi_restful.resource import HTTPMethods, ManageSignature, Resource
...
class TestResource:
    ...
    def test__include_methods__success(self, resource_instance: Resource):
        ...
        with patch.object(
            resource_instance, "_get_overridden_route_handlers", return_value=overridden_handlers
        ), patch.object(
            resource_instance, "__get_default_value_of_route_kwargs__", return_value={}
        ) as mock_1, patch.object(
            resource_instance, "_create_new_route_handler"
        ) as mock_2, patch.object(
            resource_instance.router, "add_api_route"
        ) as mock_3:
